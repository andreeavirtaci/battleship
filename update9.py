...
class JocBattleship:
        ...
        for i in range(8):
            for j in range(8):
                self.canvas.create_rectangle(j * 50 + 50, i * 50 + 50, (j + 1) * 50 + 50, (i + 1) * 50 + 50,
                                             fill="white")
                if jucator == self.jucator_activ:
                    if self.faza == "atac":
                        if jucator.harta_atac[i][j] == 'X':
                            self.canvas.create_text(j * 50 + 75, i * 50 + 75, text="X", fill="black",
                                                    font=("Arial", 24))
                        elif jucator.harta_atac[i][j] == '0':
                            self.canvas.create_text(j * 50 + 75, i * 50 + 75, text="0", fill="black",
                                                    font=("Arial", 24))
                    elif self.faza == "plasare":
                        if jucator.harta[i][j] == 'O':
                            culoare = "blue"
                            self.canvas.create_rectangle(j * 50 + 50, i * 50 + 50, (j + 1) * 50 + 50, (i + 1) * 50 + 50,
                                                         fill=culoare)
                elif jucator == self.jucator_inactiv:
                    if self.faza == "atac":
                        if jucator.harta_atac[i][j] == 'X':
                            self.canvas.create_text(j * 50 + 75, i * 50 + 75, text="X", fill="black",
                                                    font=("Arial", 24))
    ...
    def schimba_jucator(self):
        ...
        if self.jucator_activ == self.jucator2:
            if self.faza == "atac":
                self.root.after(2000, self.ataca_random_computer)
            elif self.faza == "plasare":
                self.faza = "atac"
                self.placare_frame.pack_forget()
                self.creeaza_interfata_atac()

    def ataca_random_computer(self):
        if self.faza == "atac" and self.jucator_activ == self.jucator2:
            x, y = self.selecteaza_pozitie_aleatorie_valabila()
            self.fereastra_atac(x, y)
            if self.verifica_castigator(self.jucator_inactiv):
                self.afiseaza_mesaj_castigator()
            else:
                if self.jucator_activ == self.jucator2:  # Asigură că atacul computerului continuă până când ratează
                    self.root.after(2000, self.ataca_random_computer)
                else:
                    self.root.after(2000, self.schimba_jucator)

    def selecteaza_pozitie_aleatorie_valabila(self):
        while True:
            x = random.randint(0, 7)
            y = random.randint(0, 7)
            if self.jucator_activ.harta_atac[x][y] not in ['X', '0']:
                break
        return x, y
